...
SWEP.Instructions   = "For scripting new vehicles n shieeet"
...
if SERVER then 
	util.AddNetworkString("PeanusWeanus")
	util.AddNetworkString("NOVA_DumpVehicleTable")
end
...
local function AppendSubTable( tab, subofsub )
	local tx = "{ \n"
	if subofsub then tx = "{ " end
	for k, v in pairs( tab ) do
--		print( type( k ) )
		if isstring( k ) then
			if !subofsub then tx = tx.."	" end
			tx = tx.."[\""..k.."\"] = "
		else
			tx = tx.."		["..k.."] = "
		end

		if isvector( v ) then tx = tx.."Vector( "..math.Round(v.x, 4)..", "..math.Round(v.y, 4)..", "..math.Round(v.z, 4).." ),\n"
		elseif istable( v ) then
			tx = tx..AppendSubTable( v, true )
...
local function PrintTableRaw( cls, tab )
	local finished = "t[\""..cls.."\"] = {\n"

	for k, v in pairs( tab ) do
		local tx = "	[\""..k.."\"] = "
		if isvector( v ) then tx = tx.."Vector( "..math.Round(v.x, 4)..", "..math.Round(v.y, 4)..", "..math.Round(v.z, 4).." ), "
		elseif istable( v ) then
			tx = tx..AppendSubTable( v )
		elseif isstring( v ) then
			tx = tx.."\""..v.."\","
		else 
			tx = tx..v..","
...
if CLIENT then
	net.Receive( "NOVA_DumpVehicleTable", function( l )
		local cls = net.ReadString()
		local tab = net.ReadTable()

		print( "t[\""..cls.."\"] = {" )
		PrintTableRaw( cls, tab )
		print( "}" )
	end )
end
...
--	car:SetEnginePowerMultiplier( 0.75 )
	net.Start( "NOVA_DumpVehicleTable" )
	net.WriteString( car:GetVehicleClass() )
	net.WriteTable( CMOD_Cars[car:GetVehicleClass()] )
	net.Send( self.Owner )
